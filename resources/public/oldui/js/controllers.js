...
function demoOutcome(demoStats) {
  if (demoStats.winner == "2") outcome = "Team A wins";
  else if (demoStats.winner == "3") outcome = "Team B wins";
  else outcome = "Draw";
  return outcome + "! ";
}
...
function date2timestamp(date) {
  if (date) return Math.round(date / 1000);
  return null;
}
  ...
  if (tooltip != "") {
    ...
    if (banTimestamp >= demoTimestamp)
      return (
        tooltip +
        " (" +
        bannedSinceDemo(banTimestamp, demoTimestamp) +
...
function getRequestFilters($scope) {
  ...
  if (teammates.length > 0) params["teammates"] = teammates.join();
  return params;
}
...
function filtersChanged($scope, $http, refreshStatsOnly) {
  ...
  $http
    .get(serverUrl + "/player/" + steamid + "/stats", { params: params })
    .success(function (data) {
      $scope.stats = data;
      $scope.stats.weapons.forEach(function (p) {
        p.display_name = weapon_prettify[p.name]
          ? weapon_prettify[p.name]
          : p.name.toUpperCase();
        p.hs_percent = (p.hs / p.kills) * 100;
      });
    });
  if (!refreshStatsOnly) {
    $scope.tabs.demos.status =
      $scope.tabs.charts.status =
      $scope.tabs.banned.status =
        null;
  ...
  function (
    ...
    $scope.filterBanned = function () {
      $scope.filteredBanned = $scope.banned.filter(function (p) {
        if ($scope.opponentsOnly) return p.opponent;
        return true;
      });
    };
    ...
    $scope.getPlayersInfo = function (missingPlayers) {
      ...
      $http
        .get(getPlayerSummaries(missingPlayers))
    ...
    $scope.linkToTick = function (spec, number) {
      ...
      return (
        "<a ng-click=\"watchDemo('" +
        $scope.theDemo.demoid +
        "', '" +
        steamid +
        "', " +
        (round ? number : "nil") +
    ...
    $scope.updateDemoNotesView = function () {
      if (typeof $scope.notesControls.demoNotesInput != undefined) {
        var temp = $compile(
          "<span>" +
            $scope.addLinks($scope.notesControls.demoNotesInput) +
            "</span>"
        )($scope);
        var notesViewNode = angular.element(
    ...
    $scope.updateDemoNotes2 = function () {
      if ($rootScope.isAuthorized)
        $http
          .post(serverUrl + "/demo/" + $scope.visibleDemo + "/notes", {
            notes: $scope.notesControls.demoNotesInput,
          })
          .success(function () {
    ...
    $scope.doMakeVisible = function (demoid, round) {
      ...
      $http
        .get(serverUrl + "/demo/" + demoid + "/notes")
        .success(function (response) {
          if ($scope.visibleDemo == demoid) {
            ...
            if (response.notes) $scope.notesVisible = true;
            setTimeout(function () {
              $scope.updateDemoNotesView();
            }, 100);
          }
    ...
    $scope.makeVisible = function (demoid, round) {
      ...
      if ($scope.visibleDemo != demoid) {
        if (!$scope.demoStats[demoid]) {
          $http
            .get(serverUrl + "/demo/" + demoid + "/stats")
            .success(function (data) {
        ...
        } else $scope.doMakeVisible(demoid, round);
    ...
    $scope.addTeammate = function (teammate) {
      if (
        $scope.filterTeammates.indexOf(teammate) != -1 ||
        $scope.filterTeammates.length == 4
      )
        return;
    ...
    $scope.$watch("startDate", function () {
      ...
      $scope.filterDemos.startDate = date2timestamp($scope.startDate);
      if ($changed) filtersChanged($scope, $http);
    });
    ...
    $scope.setFolder = function (folder) {
      if ($scope.filterDemos.folder == folder) return;
    ...
    var loadChartData = function () {
      var params = getRequestFilters($scope);
      $http
        .get(serverUrl + "/player/" + steamid + "/maps/statistics", {
          params: params,
        })
        .success(function (data) {
          ...
          for (var key in data) {
            ...
            $scope.mapsPlayedConfig.series[0].data.push({
              name: key,
              y: data[key].played,
            });
            ...
            var started_ct =
              data[key].won_starting_ct + data[key].lost_starting_ct;
            $scope.mapsWinConfig.series[0].data.push({
              name: key,
              y: t_won + t_lost ? ((t_won / started_t) * 100) | 0 : null,
              played: started_t,
              won: t_won,
            });
            $scope.mapsWinConfig.series[1].data.push({
              name: key,
              y: ((data[key].won_starting_ct / started_ct) * 100) | 0,
              played: started_ct,
              won: data[key].won_starting_ct,
            });
            $scope.mapsWinConfig.series[2].data.push({
              name: key,
              y: ((data[key].won / games) * 100) | 0,
              played: games,
              won: data[key].won,
            });
            $scope.roundsWinConfig.series[0].data.push({
              name: key,
              y: data[key].t_rounds
                ? ((data[key].t_rounds_won / data[key].t_rounds) * 100) | 0
                : null,
              played: data[key].t_rounds,
              won: data[key].t_rounds_won,
            });
            $scope.roundsWinConfig.series[1].data.push({
              name: key,
              y: data[key].ct_rounds
                ? ((data[key].ct_rounds_won / data[key].ct_rounds) * 100) | 0
                : null,
          ...
          $http
            .get(serverUrl + "/player/" + steamid + "/rank_data", {
              params: params,
            })
            .success(function (data) {
              $scope.rankData = data;
      ...
      $http
        .get(serverUrl + "/player/" + steamid + "/banned/statistics", {
          params: params,
        })
        .success(function (data) {
          var d = [],
    ...
    var getDemos = function () {
      ...
      if ($rootScope.config.demos_per_page)
        params["offset"] =
          ($scope.demoPages.currentPage - 1) * $rootScope.config.demos_per_page;
      else params["offset"] = 0;
      params["limit"] = $rootScope.config.demos_per_page;
      $http
        .get(serverUrl + "/player/" + steamid + "/demos", { params: params })
        .success(function (data) {
    ...
    $scope.tabs = {
      ...
      search_round: {
        heading: "Search Round",
        content: "search_round",
        icon: "search",
        status: "loaded",
      },
      charts: {
    ...
    $scope.loadTab = function ($tab) {
      ...
      if ($tab.status || $tab.load == undefined) return;
      $tab.status = "loading";
      $tab.load();
    };
    ...
    $scope.setRankChartXAxis = function (what) {
      ...
      $scope.rankData.forEach(function (m) {
        ...
        if (m.mm_rank_update != null && m.mm_rank_update.rank_new != 0) {
          d.push({
            x: $scope.rankChartXAxis == "timestamp" ? m.timestamp : i,
            y: m.mm_rank_update.rank_new,
            date: m.date,
            old: m.mm_rank_update.rank_old,
            wins: m.mm_rank_update.num_wins,
          });
        }
      });
      $scope.rankConfig.series[0].data = d;
      $scope.rankConfig.options.xAxis.labels.formatter =
        $scope.rankChartXAxis == "timestamp"
          ? $scope.rankConfig.options.xAxis.labels.formatter_time
          : null;
    };
    ...
    var pointFormat =
      '<span style="color:{point.color}">\u25CF</span> {series.name}: <b>{point.y}%</b> ({point.won}/{point.played})<br/>';
    $scope.mapsWinConfig = {
      options: {
        chart: {
          backgroundColor: "transparent",
    ...
    $scope.roundsWinConfig = {
      ...
      xAxis: { categories: [] },
      series: [{ name: "T Rounds" }, { name: "CT Rounds" }],
    };
    ...
    $scope.rankConfig = {
      options: {
        ...
        xAxis: {
          labels: {
            formatter_time: function () {
              return timestamp2date(this.value, true);
            },
            formatter: null,
          },
        },
        yAxis: {
          ...
          labels: {
            useHTML: true,
            formatter: function () {
              return rankImg(this.value);
            },
          },
        },
        tooltip: {
          pointFormatter: rankTooltipFormatter,
          headerFormat: "",
          useHTML: true,
        },
      },
    ...
    $scope.bannedConfig = {
      options: {
        ...
        xAxis: {
          labels: {
            formatter: function () {
              return timestamp2month(this.value);
            },
          },
        },
        yAxis: {
      ...
      series: [
        {
          name: "% Games with banned players in any team",
          data: null,
          turboThreshold: 0,
          color: "LightSkyBlue",
        },
        {
          name: "% Games with banned players in opposing team",
          data: null,
          turboThreshold: 0,
          color: "#3baff7",
        },
    ...
    $http
      .get(serverUrl + "/player/" + steamid + "/maps")
      .success(function (data) {
        $scope.playerMaps = data;
      });
    $http
      .get(serverUrl + "/player/" + steamid + "/teammates")
      .success(function (data) {
  ...
  $scope.pageChanged = function () {
    $http
      ...
      .success(function (data) {
        ...
        var steamIds = $scope.players.map(function (p) {
          return p.steamid;
        });
...
hsboxControllers.controller(
  "DemoLog",
  function ($scope, $http, $routeParams, watchDemo, downloadDemo) {
    $scope.watchDemo = watchDemo;
    $scope.downloadDemo = downloadDemo;
    demoid = $routeParams.demoid;
    $scope.playerName = function (player) {
      if (player == null) return "BOT";
      return player.name;
    };
    $http
      .get(serverUrl + "/demo/" + demoid + "/details")
      .success(function (data) {
        $scope.demo = data;
...
hsboxControllers.controller(
  ...
  function ($scope, $http, $routeParams, watchDemo, downloadDemo) {
    ...
    $scope.setOrder = function (field) {
      if ($scope.orderRounds == field) $scope.orderRounds = "-" + field;
      else $scope.orderRounds = field;
    };
    ...
    $scope.search = function () {
      ...
      params["search-string"] =
        (steamid ? steamid : "") + " " + $scope.search_string;
      $scope.searchInProgress = true;
      $scope.rounds = [];
      $http
        .get(serverUrl + "/round/search", { params: params })
        .success(function (data) {
          $scope.searchInProgress = false;
          $scope.rounds = data;
          $scope.rounds.forEach(function (r) {
            ...
            if (r.won) r.won_str = "Yes";
            else r.won_str = "No";
          });

          var steamIds = data.map(function (p) {
            return p.steamid;
          });
          var url = getPlayerSummaries(steamIds);
          $http.get(url).success(function (response) {
    ...
    $scope.kill_description = function (kill) {
      return (
        (kill.headshot ? "headshot" : "killed") +
        " with " +
        kill.weapon +
        (kill.noscope ? " noscope" : "") +
        (kill.quickscope ? " quickscope" : "") +
        (kill.smoke ? " through smoke" : "") +
        (kill.penetrated ? " through wall" : "") +
...
hsboxControllers.controller(
  ...
  function ($scope, $http, $rootScope, config) {
    ...
    $scope.invertIndexerState = function () {
      if (typeof $scope.indexerRunning === "undefined") return;
      $http
        .post(serverUrl + "/indexer", { running: !$scope.indexerRunning })
        .success(function (data) {
...
hsboxControllers.controller(
  "Navbar",
  function ($scope, $http, $interval, $rootScope, config) {
